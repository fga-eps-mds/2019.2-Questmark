...
* **Controller:** parte responsável por tratar as requisições do usuário, assim realizando ações e controlando a camada de _view_ e _model_ para chegar ao resultado desejado conforme as regras de negócio do projeto. 
...
se de forma extramente eficiente para qualquer aplicação Web baseado no Express. Com um conjunto de estratégias, com suporte na autenticação para facebook,twitter e mais. A Estratégia adotado no Questmark foi a local.
...
O usuário da plataforma, quando deslogado, é capaz de responder questionários de terceiros.
